...
def matrix_mul(m_a, m_b):
    ...
    if not all(isinstance(row, list)r row in m_b):
    ...
    if m_a == [] or m_a == [[]]:
  == [] or m_b == [[]]:
        raise ValueErrment, int) or isinstance(element, float))
    ...
    if not all((isinstance(element, int) or isinstance(element, float))
        ...
        raise TypeError("m_b should contain only integers oow in m_a):
        raise TypeError("each row of m_a must should be of the same size")
    if not all(len(row) == len(m_b[0]) fo:set number
